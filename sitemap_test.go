...
func TestSitemapGeneration(t *testing.T) {
	...
	stylesheetURL := "https://www.example.com/sitemaps/sitemap.xsl"
	...
	err := sm.Write(baseSitemapURL, stylesheetURL)
	if err != nil {
	...
	// Check if the stylesheet URL is correctly included
	if !strings.Contains(string(data), `href="`+stylesheetURL+`"`) {
		t.Fatalf("Stylesheet URL not correctly included in sitemap index")
	}
